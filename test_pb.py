...
def run_inference_pb(wanted_words, sample_rate, clip_duration_ms,
    ...
    softmax = sess.graph.get_tensor_by_name("prediction:0")
    ...
    if(FLAGS.training):
        ...
        tf.logging.info('Training accuracy = %.2f%% (N=%d)' % (total_accuracy * 100, set_size))
    ...
    for i in range(0, set_size):
        ...
        #     print(type(validation_data))    # <class 'bytes'>
...
def run_inference_pb_no_mfcc(wanted_words, sample_rate, clip_duration_ms,
    ...
    # sess.run(tf.global_variables_initializer())   # 需要初始化才能运行说明有参数没保存下来？可是为什么tflite能用
...
def main(_):
    ...
    if FLAGS.embed_mfcc == True:
        ...
        run_inference_pb_no_mfcc(FLAGS.wanted_words, FLAGS.sample_rate,
            FLAGS.clip_duration_ms, FLAGS.window_size_ms,
            FLAGS.window_stride_ms, FLAGS.dct_coefficient_count,
            FLAGS.model_architecture, FLAGS.model_size_info)

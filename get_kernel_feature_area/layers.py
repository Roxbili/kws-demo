...
def extract_sliding_windows(x, ksize, pad, stride, floor_first=True):
    ...
    y = np.ndarray((n, h2, w2, kh, kw, c),
                   dtype=x.dtype,
                   buffer=x.data,
                   offset=array_offset(x),  # 0
                   strides=y_strides)
    return y


def conv2d(x, w, pad='SAME', stride=(1, 1), save_name=None):
    ...
    if save_name is not None:
        path = os.path.join('get_kernel_feature_area/log', save_name + '.txt')
        if os.path.exists(path) == True:
            os.remove(path)
        with open(path, 'a') as f:
            print('\nShape: {}\n'.format(x.shape), file=f)
            np.savetxt(path, x, fmt='%4d')
...
def depthwise_conv2d(x, w, pad="SAME", stride=(1,1), save_name=None):
    y = []
...
def pooling(mat,ksize,method='max',pad=False):
    ...
    <mat>: ndarray, input array to pool. [i,h,w,c]
...
if __name__ == "__main__":
    # a = np.array([[1.,2.], [1.,2.]])
    a = np.random.rand(1,4,6,32)
    re = pooling(a, (2,2), method='mean')
    print(re.shape)
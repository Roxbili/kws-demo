...
# from nas_model_zynq import create_mbnetv3_cnn_model
...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
    ...
    return create_ds_cnn_model(fingerprint_input, model_settings, 
      ...
      return create_quant_ds_cnn_model(fingerprint_input, model_settings,
                                       model_size_info, is_training)
  elif model_architecture == 'mobilenet-v3':
    return create_mbnetv3_cnn_model(fingerprint_input, model_settings, is_training)
  else:
    raise Exception('model_architecture argument "' + model_architecture +
                    '" not recognized, should be one of "single_fc", "conv",' +
                    ' "low_latency_conv", "low_latency_svdf",'+ 
                    ' "dnn", "cnn", "basic_lstm", "lstm",'+
                    ' "gru", "crnn", "ds_cnn" or "mobilenet-v3')


def load_variables_from_checkpoint(sess, start_checkpoint, vars=None):

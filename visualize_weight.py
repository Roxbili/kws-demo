...
def main(_):
	...
	logits, dropout_prob = models.create_model(
		...
		act_max=FLAGS.act_range,
		num_act_bits=FLAGS.num_act_bits,
		is_training=True)
	...
	if FLAGS.num_weight_bits != 32:
		def _create_quantize_op(var, quant_bits, range=None):
			if range is None:
				print("Using fake-quant ...")
				max_val = tf.reduce_max(var)
				min_val = tf.reduce_min(var)
			else:
				max_val = range - 2 ** (1 - quant_bits)
				min_val = -range

			quantized_var = tf.fake_quant_with_min_max_vars(var,
			                                     min=min_val,
			                                     max=max_val,
			                                     num_bits=quant_bits)
		...
		for var in tf.trainable_variables():
			...
			quantized_var = _create_quantize_op(var, quant_bits=FLAGS.num_weight_bits,
			                                                       range=FLAGS.weight_range)
			assign_weight_op_ = tf.assign(var, quantized_var)
			assign_weight_op.append(assign_weight_op_)
			recover_weight_op_ = tf.assign(var, var)
			recover_weight_op.append(recover_weight_op_)

		# Create the back propagation and training evaluation machinery in the graph. Use quantized weights.
		with tf.control_dependencies(assign_weight_op):
			with tf.name_scope('cross_entropy'):
				cross_entropy_mean = tf.reduce_mean(
					tf.nn.softmax_cross_entropy_with_logits(
						labels=ground_truth_input, logits=logits))
		control_dependencies += recover_weight_op
	else:
		with tf.name_scope('cross_entropy'):
			cross_entropy_mean = tf.reduce_mean(
				tf.nn.softmax_cross_entropy_with_logits(
...
if __name__ == '__main__':
	...
	parser.add_argument("--num_act_bits", type=float, default=8,
						help="Activation quantization bits")
	parser.add_argument("--num_weight_bits", type=float, default=8,
						help="Weight quantization bits")
	parser.add_argument("--weight_range", type=float, default=0.5,
						help="Range for weight.")
	parser.add_argument("--act_range", type=float, default=1.0,
						help="Range for activation")
